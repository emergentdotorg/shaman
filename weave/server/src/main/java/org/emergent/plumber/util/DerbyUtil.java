/*
 *
 * Derby - Class org.emergent.plumber.util.JDBC
 *
...
public class DerbyUtil {
  ...
  public static void dropSchema(DatabaseMetaData dmd, String schema) throws SQLException {
    ...
    while (table_rs.next()) {
      ...
      while (rs.next()) {
        ...
        String ddl = "ALTER TABLE " +
            DerbyUtil.escape(fkSchema, fkTable) +
            " DROP FOREIGN KEY " +
            DerbyUtil.escape(fkName);
        s.executeUpdate(ddl);
      }
      rs.close();
    }
    ...
    if (!schema.equals("APP")) {
      s.executeUpdate("DROP SCHEMA " + DerbyUtil.escape(schema) + " RESTRICT");
    }
    conn.commit();
    s.close();
  }
  ...
  private static void assertFullResultSetMinion(
    ...
    for (rows = 0; rs.next(); rows++) {
      ...
      // DERBY-159.
      if (DerbyUtil.BaseJDBCTestCase.TestConfiguration_getCurrent_getJDBCClient_isEmbedded() &&
          warnings != null) {
  ...
  public static void assertCurrentSchema(Connection con, String schema)
    ...
    try {
      DerbyUtil.assertSingleValueResultSet(
...
//        rtsp.assertSequence(sequence);
  ...
  private static class BaseJDBCTestCase {
    public static void assertEquals(Blob blob, Blob blob1) {
    }

    public static void assertEquals(Clob blob, Clob blob1) {
    }

    public static boolean TestConfiguration_getCurrent_getJDBCClient_isEmbedded() {
      return false;
    }
  }
  ...
  private static class Assert {
    ...
    public static void assertEquals(String s, int i1, int i2) {
    }
