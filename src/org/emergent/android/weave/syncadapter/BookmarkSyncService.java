 ...
 */
public class BookmarkSyncService extends AbstractSyncService {
  ...
  protected AbstractSyncAdapter getSyncAdapter() {
    synchronized (sm_syncAdapterLock) {
      if (sm_syncAdapter == null) {
        sm_syncAdapter = new BookmarkSyncAdapter(getApplicationContext(), true);
      }
    }
    return sm_syncAdapter;
  }

  private static class BookmarkSyncAdapter extends AbstractSyncAdapter {

    public BookmarkSyncAdapter(Context context, boolean autoInitialize) {
      super(context, autoInitialize, Bookmarks.UPDATER);
    }
  }
}
